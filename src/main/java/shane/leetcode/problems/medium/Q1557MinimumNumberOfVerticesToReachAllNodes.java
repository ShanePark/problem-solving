...
//                .containsExactlyInAnyOrder(14, 47);
    ...
    public List<Integer> findSmallestSetOfVertices(int n, List<List<Integer>> edges) {
        ...
        for (List<Integer> edge : edges) {
            Node parent = map.get(edge.get(0));
            Node child = map.get(edge.get(1));
            parent.addChild(child);
        }

        for (int i = 1; i <= n; i++) {
            if (isPossibleWithElements(i, map, new ArrayList<>(), 0)) {
                return answer;
            }
        }
    ...
    private boolean isPossibleWithElements(int size, Map<Integer, Node> map, List<Node> nodes, int index) {
        if (index > map.size() || size - nodes.size() > map.size() - index)
            return (answer != null);

        if (nodes.size() == size || index == map.size()) {
            ...
            if (children.size() == map.size()) {
                answer = nodes.stream().map(n -> n.num).collect(Collectors.toList());
                return true;
            } else {
                return false;
            }
        ...
        copy.add(map.get(index));
        isPossibleWithElements(size, map, copy, index + 1);

        // withoutIndexed
        isPossibleWithElements(size, map, nodes, index + 1);
    ...
    class Node {
        ...
        public String toString() {
            return "Node{" +
                    "num=" + num +
                    ", children=" + children.stream().map(n -> n.num).collect(Collectors.toList()) +
                    ", parent=" + ((parent == null) ? "null" : parent.num) +
                    '}';
        }
    }
}
