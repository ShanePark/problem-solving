 ...
 * TLE
 */
public class Q1857LargestColorValueInADirectedGraph2 {
    ...
    private void dfs(Node node, int[] cur, Set<Node> visited) {
        if (node.next.isEmpty()) {
            max = Math.max(max, Arrays.stream(cur).max().getAsInt());
        }
