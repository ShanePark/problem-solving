package shane.leetcode.contest.weekly.w283;

import org.assertj.core.api.Assertions;
import org.junit.jupiter.api.Test;

import java.util.Arrays;

/**
 * TLE
 */
@SuppressWarnings("ALL")
public class Q2195AppendKIntegersWithMinimalSum4 {

    @Test
    void test() {
        Assertions.assertThat(minimalKSum(new int[]{1, 4, 25, 10, 25}, 2)).isEqualTo(5);
        Assertions.assertThat(minimalKSum(new int[]{5, 6}, 6)).isEqualTo(25);
        Assertions.assertThat(minimalKSum(new int[]{53, 41, 90, 33, 84, 26, 50, 32, 63, 47, 66, 43, 29, 88, 71, 28, 83}, 76)).isEqualTo(3444);
        Assertions.assertThat(minimalKSum(new int[]{1}, 1000000000)).isEqualTo(500000001500000000L);
    }

    public long minimalKSum(int[] nums, int k) {
        int i = 1;
        int cnt = 0;
        long sum = 0;

        Arrays.sort(nums);
        int arrIndex = 0;

        while (cnt < k) {
            while (arrIndex < nums.length - 1 && nums[arrIndex] < i) {
                arrIndex++;
            }
            if (nums[arrIndex] != i) {
                cnt++;
                sum += i;
            }
            i++;
        }
        return sum;
    }
}
