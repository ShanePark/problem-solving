...
/**
 * Runtime 444 ms Beats 8.56%
 * Memory 141.8 MB Beats 46.39%
 */
public class Q2246LongestPathWithDifferentAdjacentCharacters {
    ...
    private int dfs(Map<Integer, List<Integer>> map, String s, int cur) {
        ...
        for (Integer next : map.get(cur)) {
            ...
            if (s.charAt(cur) != nextChar) {
                subs.add(dist);
            }
        }

        List<Integer> distances = subs.stream()
                .sorted(Comparator.reverseOrder())
                .collect(Collectors.toList());
        if (distances.size() >= 2) {
            max = Math.max(max, distances.get(0) + distances.get(1) + 1);
        }

        int distance = distances.isEmpty() ? 1 : distances.get(0) + 1;
