...
/**
 * couldn't solve it. idea from community
 * Runtime 36 ms Beats 67.11%
 * Memory 68.2 MB Beats 74.91%
 */
public class Q886PossibleBipartition2 {
    ...
    private boolean dfs(List<Integer>[] graph, Boolean[] groups, int cur, boolean group) {
        groups[cur] = group;

        for (int node : graph[cur]) {
            if (groups[node] == null) {
                if (!dfs(graph, groups, node, !group))
                    return false;
            } else if (groups[node] == group) {
                return false;
            }
        }
        return true;
