...
public class Q1048LongestStringChain2 {
    ...
    public int longestStrChain(String[] words) {
        ...
        for (int i = 0; i < words.length; i++) {
            ...
            Set<Q> toAdd = new HashSet<>();
            for (Q q : set) {
                if (q.lengthOfWord == length - 1 && valid(q.lastWord, word)) {
                    toAdd.add(new Q(word, length, q.count + 1));
                }
            }
            set.addAll(toAdd);
            set.add(new Q(word, length, 1));
        }
