...
public class Q1696JumpGameVI2 {
    ...
    public int maxResult(int[] nums, int k) {
        PriorityQueue<Step> pq = new PriorityQueue<>(Comparator.comparingInt(o -> -o.score));
        int sum = 0;
        int right = 0;
        for (int i = 1; i < nums.length - 1; i++) {
            pq.offer(new Step(i, nums[i]));
            if (pq.size() == k || nums[i] >= 0) {
                Step poll = pq.poll();
                sum += poll.score;
                pq.clear();
                right = poll.index;
        ...
        if (right + k < nums.length - 1) {
            int pebble = nums[nums.length - 2];
            for (int i = right + 1; i < nums.length - 1; i++) {
                pebble = Math.max(pebble, nums[i]);
            }
            sum += pebble;
        }
    ...
    class Step {
        public Step(int index, int score) {
            this.index = index;
            this.score = score;
        }
