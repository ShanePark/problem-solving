...
public class Q834SumOfDistancesInTree3 {
    ...
    public int[] sumOfDistancesInTree(int n, int[][] edges) {
        ...
        for (int i = 0; i < edges.length; i++) {
            graph[edges[i][0]].add(new Node(edges[i][1], 0, 0));
            graph[edges[i][1]].add(new Node(edges[i][0], 0, 0));
        }

        int[] answer = new int[n];
        for (int i = 0; i < n; i++) {
            answer[i] = dfs(graph, i, new boolean[n]).sum;
        }
        return answer;
    }

    private Node dfs(List<Node>[] graph, int i, boolean[] visited) {
        int sum = 0;
        int count = 1;
        ...
        for (Node next : graph[i]) {
            ...
            if (next.sum == 0 && next.count == 0) {
                Node dfs = dfs(graph, next.index, visited);
                next.sum = dfs.sum;
                next.count = dfs.count;
            }
            sum += next.sum + next.count;
            count += next.count;
        }

        visited[i] = false;
        return new Node(-1, sum, count);
    }
    ...
    class Node {
        ...
        public Node(int index, int sum, int count) {
            this.index = index;
            this.sum = sum;
            this.count = count;
        }
