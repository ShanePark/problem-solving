 ...
 * TLE
 */
public class Q2192AllAncestorsOfANodeInADirectedAcyclicGraph2 {
    ...
    public List<List<Integer>> getAncestors(int n, int[][] edges) {
        ...
        for (int i = 0; i < parents.size(); i++) {
            ...
            for (Node parent : node.parents) {
                ...
                allParents.add(parent.num);
                parents.get(i).addAll(allParents);
                parents.set(i, parents.get(i).stream().distinct().sorted().collect(Collectors.toList()));
            }
        }
    ...
    private void findAllParents(Node parent, Set<Integer> allParents) {
        for (Node node : parent.parents) {
            allParents.add(node.num);
            findAllParents(node, allParents);
        }
    }
