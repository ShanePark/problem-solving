...
/**
 * Runtime 175 ms Beats 7.94%
 * Memory 59.3 MB Beats 25.40%
 */
public class Q1964FindTheLongestValidObstacleCourseAtEachPosition3 {
    ...
    public int[] longestObstacleCourseAtEachPosition(int[] obstacles) {
        int[] minObstaclesByLength = IntStream.generate(() -> Integer.MAX_VALUE)
                .limit(obstacles.length)
                .toArray();

        return IntStream.range(0, obstacles.length)
                .map(i -> {
                    int obstacle = obstacles[i];
                    int index = binarySearch(minObstaclesByLength, obstacle);
                    minObstaclesByLength[index] = obstacle;
                    return index + 1;
                }).toArray();
    }

    private int binarySearch(int[] minObstaclesByLength, int obstacle) {
        int left = 0;
        int right = minObstaclesByLength.length - 1;
        while (left < right) {
            int mid = (left + right) / 2;
            if (minObstaclesByLength[mid] <= obstacle) {
                left = mid + 1;
            } else {
                right = mid;
            }
