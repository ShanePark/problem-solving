...
public class Q1751MaximumNumberOfEventsThatCanBeAttendedII2 {
    ...
    public int maxValue(int[][] events, int k) {
        ...
        List<Event> list = Arrays.stream(events)
                .map(e -> new Event(e))
                .sorted()
        ...
        for (int i = LENGTH - 1; i >= 0; i--) {
            ...
            int before = binarySearch(list, event.end);
            for (int k_ = 1; k_ <= k; k_++) {
                int skip = dp[k_][i + 1];
                int included = event.value + dp[k_ - 1][before];
                dp[k_][i] = Math.max(skip, included);
            }
        }
        return dp[k][0];
    }

    private int binarySearch(List<Event> events, int target) {
        int left = 0;
        int right = events.size();
        while (left < right) {
            int mid = (left + right) / 2;
            if (events.get(mid).start <= target) {
                left = mid + 1;
            } else {
                right = mid;
            }
